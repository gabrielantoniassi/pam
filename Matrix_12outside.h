...
template<const int mdepth, const int mcols>
SMatrix<Scalar, rows, mdepth> mul(const SMatrix<Scalar, lhsm_rows, lhsm_cols>& lhs, const SMatrix<Scalar, mdepth, mcols>& rhs)
{
    SMatrix<Scalar, lhsm_rows, mdepth> res;
    for(int i = 0; i < rows; i++)
    {
        for(int j = 0; j < mcols; j++)
        {
            ...
            {
                res.m_data[StorageOrder == RowMajor ? i*mdepth + j : j*lhsm_rows + i] += lhs.m_data[StorageOrder == RowMajor ? i*mdepth + k : k*lhsm_rows + i]*rhs.m_data[StorageOrder == RowMajor ? k*mcols + j : j*mdepth + k];
            }
        }
    }
    return res;
...
template<>
SMatrix<Scalar, 12, 12> mul<12, 12>(const SMatrix<Scalar, 12, 12>& lhs, const SMatrix<Scalar, 12, 12>& rhs)
{
