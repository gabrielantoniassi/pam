...
public:
    ...
    SMatrix<Scalar, rows, mRHS_cols> hyb(const SMatrix<Scalar, cols, mRHS_cols>& rhs);
    //====================================================================================================
...
template<int mRHS_cols>
SMatrix<Scalar, rows, mRHS_cols> SMatrix<Scalar, rows, cols, StorageOrder>::hyb(const SMatrix<Scalar, cols, mRHS_cols>& rhs)
{
	// do nothing
	return rhs;
}
...
template<>
template<>
SMatrix<double, 4, 4> SMatrix<double, 4, 4, RowMajor>::hyb<4>(const SMatrix<double, 4, 4>& rhs)  // lhs(this) is RowMajor and rhs is RowMajor
{
    SMatrix<double, 4, 4> res;
    hhybrid((double*)this->m_data, (double*)rhs.m_data, (double*)res.m_data);
    return res;
}
